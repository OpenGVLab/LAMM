...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        with torch.autograd.profiler.record_function("EnsembleModel: forward_encoder"):
            encoder_outs = self.model.forward_encoder(net_input) # None
        ...
        # initialize buffers
        scores = ( # generated tokens
            torch.zeros(bsz * beam_size, max_len + 1).to(src_tokens).float() 
        )  # +1 for eos; pad is never chosen for scoring
        tokens = ( # generated tokens, filled with pad
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            with torch.autograd.profiler.record_function(
                ...
                if "img_src_tokens" in net_input and step == 0:
                    ...
                    if len(img_token_size) == 5: # few-shot setting, [B, K-shot, D(3, 244, 244)] default setting
                    ...
                    img_features =  self.model.models[0].get_image_representation(img_tokens) # [bsz*img_token_len, img_feature_dim]
                    first_src_tokens = sample['net_input']['src_tokens'].unsqueeze(1).repeat(1, beam_size, 1).view(bsz*beam_size, -1) # [bsz*beam_size, token_len]
                    img_feature_dim = img_features.size(-1)
                    first_img_features = img_features.view(bsz, -1, img_feature_dim).unsqueeze(1).repeat(1, beam_size, 1, 1).view(-1, img_feature_dim) # [bsz*img_token_len*beam_size, img_feature_dim]
