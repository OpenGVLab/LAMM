...
class TestMplugOwl(TestBase):
    ...
    def build_conversation(self, idx, image_list, prompt, CoT_answer_list=None, batch_answers=None, **kwargs):
        ...
        outputs = self.processor(text=[prompts])
    ...
    def build_input_image(self, image_list):
        ...
        for image in image_list:
            image_tensor = self.image_processor(image, return_tensors='pt').pixel_values.to(self.device, dtype=self.dtype)
    ...
    def do_generate(self, 
        image_list, # num_image * image 
        prompt_tensors, 
        max_new_tokens,
        **kwargs):
        prompt_tensors["pixel_values"] = image_list.unsqueeze(0)
        generate_kwargs = {
            'do_sample': True,
            'top_k': 5,
        ...
        with torch.no_grad():
            res = self.model.multiple_generate(**prompt_tensors, **generate_kwargs)
        outputs = [self.tokenizer.decode(output, skip_special_tokens=True) for output in res.tolist()]
        return outputs

    def do_ppl1(self, batch_images, batch_prompt, batch_answers, batch_options, CoT_list = None, calib=None, **kwargs):
        template_token_list = []
        answer_start_indices = []
        answer_end_indices = []
        answer_token_list = []
        prompts = batch_prompt
        for idx, (template, option) in enumerate(zip(batch_answers, batch_options)):
